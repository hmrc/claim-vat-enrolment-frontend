...
import uk.gov.hmrc.http.{HeaderCarrier, InternalServerException, PreconditionFailedException, UnprocessableEntityException}
...
//      case Some(internalId) ~ e ~ Some(User) ~ Some(Credentials(credId, "GovernmentGateway")) ~ Some(groupId)  if !e.enrolments.exists(_.key == mtdEnrolment) =>
      ...
      case _ =>
        errorLog(
          s"[CaptureBox5FigureController][show] - Internal ID could not be retrieved from Auth for journey: $journeyId"
        )(implicitly, request)
  ...
  override protected def transform[A](request: IdentifierRequest[A]): Future[DataRequest[A]] = {
    journeyDataRepository.getJourneyData(request.journeyId, request.userId).map {
      ...
      case None =>
        ...
        errorLog(s"[JourneyService][retrieveJourneyData] - Journey data was not found for journey ID ${request.journeyId}")
        throw new UnprocessableEntityException(s"Journey data could not be retrieved for journey ID ${request.journeyId}")
    } recover {
      case e: Exception =>
        implicit val req: Request[A] = request.request
        errorLog(s"[JourneyService][retrieveJourneyData] - Error retrieving journey data for journey ID ${request.journeyId}")
