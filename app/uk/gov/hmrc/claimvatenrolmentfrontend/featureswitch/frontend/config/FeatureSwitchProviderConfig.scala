...
package uk.gov.hmrc.claimvatenrolmentfrontend.featureswitch.frontend.config
...
class FeatureSwitchProviderConfig @Inject()(configuration: Configuration) {
  ...
  lazy val selfFeatureSwitchUrl = s"$selfBaseUrl/claim-vat-enrolment/test-only/api/feature-switches"

  lazy val selfFeatureSwitchProvider: FeatureSwitchProvider = FeatureSwitchProvider(
    id = "claim-vat-enrolment-frontend",
